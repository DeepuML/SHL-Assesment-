...
def main():
    ...
    try:
        ...
        print("=" * 70)
        print(
            f"MiniLM (Baseline):        {baseline_recall:.4f} ({baseline_recall*100:.2f}%)"
        )

        if has_gemini:
            print(
                f"Gemini (Original):        {gemini_recall:.4f} ({gemini_recall*100:.2f}%)"
            )

        print(
            f"Gemini (Enhanced):        {results['mean_recall']:.4f} ({results['mean_recall']*100:.2f}%)"
        )
        ...
        if has_gemini:
            gemini_improvement = (
                (results["mean_recall"] - gemini_recall) / gemini_recall
            ) * 100
