...
def main():
    ...
    models = [
        ...
        (
            "Gemini (Original)",
            gemini_recall,
            ((gemini_recall - baseline_recall) / baseline_recall) * 100,
            "FAILED",
        ),
        (
            "Gemini (Enhanced)",
            enhanced_recall,
            ((enhanced_recall - baseline_recall) / baseline_recall) * 100,
            "GOOD",
        ),
        (
            "Gemini (Optimized)",
            optimized_recall,
            ((optimized_recall - baseline_recall) / baseline_recall) * 100,
            "BEST",
        ),
    ]
    ...
    print(
        f"   Improvement: +{((optimized_recall - baseline_recall) / baseline_recall) * 100:.2f}%"
    )

    print("\n2. data/submission_enhanced.csv")
    print(f"   Recall@10: {enhanced_recall*100:.2f}%")
    print(
        f"   Improvement: +{((enhanced_recall - baseline_recall) / baseline_recall) * 100:.2f}%"
    )
    ...
    print(
        f"  Total Improvement: +{((optimized_recall - baseline_recall) / baseline_recall) * 100:.2f}%"
    )
    print(
        f"  Absolute Gain: +{(optimized_recall - baseline_recall)*100:.2f} percentage points"
    )
