...
class RetrievalEngine:
    ...
    def balance_results(
        ...
        for test_type in ["K", "P", "C", "A"]:
            ...
            # Take top N by score
            selected = sorted(available, key=lambda x: x["_score"], reverse=True)[
                :target
            ]
        ...
        balanced_results = sorted(
            balanced_results, key=lambda x: x["_score"], reverse=True
        )[:final_k]
    ...
    def recommend(self, query: str, k: int = 10) -> List[Dict]:
        ...
        logger.info(
            f"Query intent: K={intent['K']:.2f}, P={intent['P']:.2f}, C={intent['C']:.2f}"
        )
