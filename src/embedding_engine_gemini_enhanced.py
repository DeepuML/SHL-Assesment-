...
class EnhancedGeminiEmbeddingEngine:
    ...
    def create_rich_document_text(self, assessment: Dict) -> str:
        ...
        if description:
            ...
            parts.append(
                f"This assessment evaluates professional capabilities relevant to {name}"
            )
        ...
        if job_roles:
            ...
            if any(
                word in name.lower() for word in ["developer", "programmer", "engineer"]
            ):
                parts.append(
                    "Suitable for Job Roles: Software Developer, Engineer, Technical Specialist"
                )
            elif any(word in name.lower() for word in ["manager", "lead", "director"]):
                parts.append(
                    "Suitable for Job Roles: Manager, Team Lead, Director, Supervisor"
                )
            elif any(word in name.lower() for word in ["sales", "account", "business"]):
                parts.append(
                    "Suitable for Job Roles: Sales Professional, Account Manager, Business Development"
                )
        ...
        if test_type == "K":
            use_cases.append(
                "Use Cases: Pre-employment technical screening, skills validation, competency assessment, hiring decisions for technical roles"
            )
        elif test_type == "P":
            use_cases.append(
                "Use Cases: Personality profiling, behavioral assessment, cultural fit evaluation, leadership potential screening, team composition"
            )
        elif test_type == "C":
            use_cases.append(
                "Use Cases: Cognitive ability testing, aptitude screening, reasoning assessment, problem-solving evaluation, analytical thinking measurement"
            )
    ...
    def generate_embeddings(self, texts: List[str], batch_size: int = 10) -> np.ndarray:
        ...
        logger.info(
            f"Generating Enhanced Gemini embeddings for {len(texts)} documents..."
        )
