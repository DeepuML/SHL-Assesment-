...
class DataProcessor:
    ...
    def validate_assessment(self, assessment: Dict) -> bool:
        ...
        for field in required_fields:
            if not assessment.get(field):
                logger.warning(
                    f"Missing required field '{field}' in: {assessment.get('assessment_url', 'UNKNOWN')}"
                )
                return False
    ...
    def process(self) -> List[Dict]:
        ...
        for assessment in raw_data:
            # Clean fields
            assessment["assessment_name"] = self.clean_text(
        ...
        logger.info("=" * 60)
        logger.info(
            f"DATA PROCESSING COMPLETE: {len(processed_assessments)} assessments"
        )
        logger.info("=" * 60)
