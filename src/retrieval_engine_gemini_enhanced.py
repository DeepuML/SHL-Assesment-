...
class EnhancedGeminiRetrievalEngine:
    ...
    def expand_query(self, query: str) -> str:
        ...
        if any(
            ...
            expanded_parts.append(
                "Technical skills assessment for programming and software development"
            )
        ...
        if any(
            ...
            expanded_parts.append(
                "Personality and behavioral assessment for soft skills and work style"
            )
        ...
        if any(
            ...
            expanded_parts.append(
                "Cognitive ability and aptitude assessment for reasoning and analytical thinking"
            )

        # Sales/Business
        if any(
            word in query_lower for word in ["sales", "business", "account", "customer"]
        ):
        ...
        if "senior" in query_lower or "experienced" in query_lower:
            expanded_parts.append(
                "For experienced professionals and senior-level candidates"
            )
        elif (
            "junior" in query_lower
            or "graduate" in query_lower
            or "entry" in query_lower
        ):
    ...
    def embed_query(self, query: str) -> np.ndarray:
        ...
        result = genai.embed_content(
            model=self.model_name, content=expanded_query, task_type="retrieval_query"
        )
    ...
    def recommend(
        ...
        distances, indices = self.embedding_engine.index.search(
            query_embedding, n_candidates
        )
